...
import { getFirestore, collection, doc, setDoc, enableNetwork, writeBatch } from 'firebase/firestore';
import { firebaseConfig } from '../config/firebase';
import type { Creator, Campaign, Tip, User } from '../types/firestore';
...
const sampleCreators: Omit<Creator, 'id' | 'createdAt' | 'updatedAt'>[] = [
  {
    userId: 'creator-1',
...
const sampleCampaigns: Omit<Campaign, 'id' | 'createdAt' | 'updatedAt'>[] = [
  {
    brandId: 'brand-1',
    brandName: 'EcoCash',
...
const sampleTips: Omit<Tip, 'id' | 'createdAt' | 'updatedAt'>[] = [
  {
...
async function migrateData() {
  try {
    ...
    for (let i = 0; i < sampleCreators.length; i++) {
      ...
      const creatorDoc = doc(collection(db, 'creators'), creatorId);
    ...
    for (let i = 0; i < sampleCampaigns.length; i++) {
      ...
      const campaignDoc = doc(collection(db, 'campaigns'), campaignId);
    ...
    for (let i = 0; i < sampleTips.length; i++) {
      ...
      const tipDoc = doc(collection(db, 'tips'), tipId);
